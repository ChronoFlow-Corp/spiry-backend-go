...
const chatWithMediasQuery = `with command_medias_agg as (
    ...
    group by cmm.command_id
     ...
     result_per_command as (
             ...
             coalesce(
                             ...
                             jsonb_strip_nulls(
                                     jsonb_build_object(
                                             ...
                                             'model', coalesce(jsonb_build_object(
                                                               'id', models.id,
                                                               'name', models.name,
                                                               'min_level', models.min_level,
         ...
         from results r
                  left join models on r.model_id = models.id
    ...
    coalesce(
                    ...
                    jsonb_strip_nulls(
                            jsonb_build_object(
                                    ...
                                    'results', coalesce(rpc.results, '[]'::jsonb)
                            )
                    )
...
const chatWithMediasQuery1 = `with command_medias_agg as (
    select
        cmm.command_id,
        coalesce(
                        jsonb_agg(
